...
class LoFTLinear(nn.Module):
    def __init__(self, linear: nn.Linear, rank: int):
        ...
        self.U = nn.Parameter(torch.randn(self.out_features, rank))
        eps = 1e-6
        self.V = nn.Parameter(eps * torch.randn(self.in_features, rank))
...
class LoFTAdamW(Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            projection_U_k_inv = loft_pinv_gram(projection_U_k) # shape (r, r)
            projection_V_k_inv = loft_pinv_gram(projection_V_k) # shape (r, r)

            # Calibration matrices (PLACEHOLDERS)
            C_k_V  = (state["V_k_1"].T @ V) @ (loft_pinv_gram(projection_V_k)) # shape (r, r)
            C_k_U  = (state["U_k_1"].T @ U) @ (loft_pinv_gram(projection_U_k)) # shape (r, r)
...
plt.tight_layout()
# save the figure
plt.savefig("loft_test.png", dpi=300)
plt.show()
